(function() {
	var selector = "#scene-2 .pie-group";
	var data = [];
	var pieData;
	var sum = 0;
	...
	function renderArch() {
		...
		enter.append('x-arch-column')
			...
			.each(function(d) {
				var c = d3.rgb(colorsScale(d.name));
				$(this).on('mousemove mouseenter', function() {
					$(this).attr('scale','1.1 1.1 1.1').attr('color',c.brighter());
					$('#scene-2').css('cursor','pointer');
				}).on('mouseleave', function() {
					$(this).attr('scale','1 1 1').attr('color',colorsScale(d.name));
					$('#scene-2').css('cursor','default');
				});
			});

    ...
    function(
        ...
        function GlobalScene(container) {
            ...
            this.showHourlyNews = function(hn) {
                ...
                var time = hn.time;
                var type = hn.type;
                var author = hn.author;

                self.landMarks.forEach(function(landMark) {
                    if (landMark.material && landMark.material.map) {
                        landMark.material.map.img = null;
                        landMark.material.map.image = null;
                    }
                    landMark.children.forEach(function(child) {
                        if (child.material && child.material.map) {
                            child.material.map.img = null;
                ...
                self.hotspotMarks.forEach(function(hotspotMark) {
                    if (hotspotMark.material && hotspotMark.material.map) {
                        hotspotMark.material.map.img = null;
                        hotspotMark.material.map.image = null;
                    }
                    hotspotMark.children.forEach(function(child) {
                        if (child.material && child.material.map) {
                            child.material.map.img = null;
                            child.material.map.image = null;
                        }
                    });
                    scene.remove(hotspotMark);
                });
                console.log(hn);
                focus(function() {
                    globe.trunTo(local.log, local.lat, function() {
                        ...
                        var landMark = Landmark(local.name, '#00FFFF');
                        ...
                        var hotspotMark = HotspotMark(title, content, time, author, type);
                        ...
                        self.hotspotMarks.push(hotspotMark);
